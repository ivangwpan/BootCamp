...
class FrontController extends Controller
{
    ...
    {
        $request->validate([
            'order_id' => 'required|exists:order_forms,id',
        ]);

        $user = $request->user();
        $order = OrderForm::where('user_id', $user->id)->find($request->order_id);
        ...
        return redirect(route('user.order.list'))->with(['msg' => '你沒訂單拉']);
    }
}
