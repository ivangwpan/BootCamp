            ...
            <ul class="border">
                ...
                <div class="w-100">
                    @foreach ($itembuys as $itembuy)
                        ...
                        <li id="row{{ $itembuy->id }}"
                            class="card w-100 d-flex flex-row align-items-center justify-content-between"
                            style="width: 18rem;">
                            {{-- @dd( $itembuy->product) --}}
                            <img src="{{ asset($itembuy->product_withTrashed->image) }}" class="card-img-top w-25"
                                alt="...">
                            <div
                                class="card-body d-flex flex-row flex-wrap align-items-center w-50  justify-content-between">
                                <div>
                                ...
                                <div>
                                    ...
                                    <button type="button" class="controlBtn minusBtn"
                                        onclick="minus({{ $itembuy->id }})">-</button>
                                    <input id="product{{ $itembuy->id }}" type="number" value="{{ $itembuy->qty }}"
                                        onchange="inputQty('{{ $itembuy->id }}')" class="w-75">
                                    <button type="button" class="controlBtn plusBtn"
                                        onclick="plus({{ $itembuy->id }})">+</button>
                                </div>
                            ...
                            <div>
                                <button type="button" class="btn btn-danger"
                                    onclick="deleteCart('{{ $itembuy->id }}')">移除</button>
                            </div>
                        </li>
                    @endforeach
