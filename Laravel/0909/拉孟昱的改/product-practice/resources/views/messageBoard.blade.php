            ...
            <div class="d-flex flex-column justify-content-center flex-wrap align-items-center">
                @foreach ($messages as $item)
                    <div class="card w-100">
                        <div class="card-body">
                            <h5 class="card-title">{{ $item->id }}樓    樓主說：{{ $item->message }}</h5>
                            ...
                            <div class="d-flex column-gap-3">
                                ...
                                <button type="button" class="btn mb-2 btn-success  btn-success-self edit-mese" data-id="{{ $item->id }}" onclick="editMese({{ $item->id }})">編輯主樓</button>

                                <form action="{{ route('messageDestroy', ['id' => $item->id]) }}" method="post"
                                    class="delete-alert" data-name="{{ $item->name }}">
                                    @csrf
                                    @method('delete')
                                    <button type="submit" class="btn  btn-success  btn-success-self">刪主樓</button>
                                </form>
                            ...
                            <form id="replyForm{{ $reply->id }}" action="{{ route('replyUpdate', ['id' => $reply->id]) }}" method="POST" class="d-flex column-gap-3 align-items-center">
                                @csrf
                                @method('PUT')
                                <h5 id="replyTitle{{ $reply->id }}" class="card-title me-auto">{{ $item->id }}-{{ $reply->id }}樓：{{ $reply->replay_message }}</h5>
                                ...
                                <button class="btn-primary btn mb-1" type="button" onclick="editReply({{ $reply->id }}, this)">編輯</button>
                                <button id="updateReply{{ $reply->id }}" type="submit" class="btn-primary btn d-none">儲存更動</button>
                                <button type="button" class="btn-danger btn mb-1" onclick="deleteReply({{ $reply->id }})">刪除</button>
                                <hr>
                            </form>
                            @endforeach
                            <form action="{{ route('replayStore', ['id' => $item->id]) }}" method="post">
                                ...
                                <div class="form-floating mb-3">
                                    <input type="text" name="reply" class="form-control" id="floatingInput"
                                        placeholder="留下您的訊息">
                                    <label for="floatingInput">回覆此樓</label>
                                </div>
                    ...
                    <div class="card-body">
                        <form action="{{ route('messageStore') }}" method="post">
                            ...
                            <div class="form-floating mb-3">
                                <input type="text" name="message" class="form-control" id="floatingInput"
                                    placeholder="留下您的訊息">
                                <label for="floatingInput">新樓主要說啥呢？</label>
                            </div>
                            <button type="submit" class="btn btn-primary">蓋新樓</button>
                        </form>
                    </div>
                </div>
            </div>
